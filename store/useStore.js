  ...
  persist(
    (set, get) => ({
      ...
      setSelectedModel: async (modelName = "MED_TRP_1") => {
        ...
        const prevSelectedMaterial = get().selectedMaterial;
        const prevSelectedColor = get().selectedColor;
        let mappedPresets = {};
        if (prevModel && prevPresets && prevModel !== modelName) {
          // --- Use cached refMap from Zustand ---
          const mapper = await getReferenceMapper();
          mappedPresets = mapper.mapSelections(
            prevModel,
            modelName,
            prevPresets
          );
        ...
        // --- Preserve selectedMaterial and selectedColor if possible ---
        ...
        if (config.optionDescriptions?.Color) {
          ...
          if (colorKeys.length > 0) {
            if (
              ...
              colorKeys.includes(prevSelectedMaterial)
            ) {
              selectedMaterial = prevSelectedMaterial;
              selectedColor =
                config.colorHex?.[prevSelectedMaterial] || "#ffffff";
            } else {
      ...
      setSelectedEdgepaintColor: (color) =>
        set({ selectedEdgepaintColor: color }),
      ...
      setMeshNames: (names) =>
        set((state) => {
          ...
          deTabs.forEach((de) => {
            ...
            if (
              !opt &&
              Array.isArray(deOptions[de]) &&
              deOptions[de].length > 0
            ) {
              opt = deOptions[de][0];
            }
    ...
    {
      name: "calculus-app-store",
      partialize: (state) => ({
        selectedModel: state.selectedModel,
        modelConfig: state.modelConfig,
        meshNames: state.meshNames,
        activeLayerPresets: state.activeLayerPresets,
        selectedTab: state.selectedTab,
        selectedDEOption: state.selectedDEOption,
        selectedMaterial: state.selectedMaterial,
        selectedColor: state.selectedColor,
        colorSelected: state.colorSelected,
        selectedStitchColor: state.selectedStitchColor,
        selectedEdgepaintColor: state.selectedEdgepaintColor,
        visibleParts: state.visibleParts,
        rotationY: state.rotationY,
        productDescription: state.productDescription,
        // Add more if you want to persist more state
      }),
      storage: {
